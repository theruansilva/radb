...
impl AdbServer {
    ...
    pub async fn send_command(stream: &mut TcpStream, command: &str) -> Result<String> {
        ...
        if &status == b"OKAY" {
            ...
            Err(AdbError::Protocol(format!("ADB server command failed: {reason}")))
        } else {
...
impl AdbBinary {
    pub fn find_adb_binary() -> Result<PathBuf> {
        ...
        if let Ok(android_home) = env::var("ANDROID_HOME") {
            let candidate = PathBuf::from(android_home).join("platform-tools").join("adb");
            if candidate.exists() {
                return Ok(candidate);
            }
        }
        ...
        if let Ok(android_sdk_root) = env::var("ANDROID_SDK_ROOT") {
            let candidate = PathBuf::from(android_sdk_root).join("platform-tools").join("adb");
            if candidate.exists() {
                return Ok(candidate);
            }
        }
