...
impl AdbSync {
    pub async fn push<W: AsyncWrite + Unpin + Send + 'static>(
        ...
        Self::push_bytes(stream, &file_data, remote_path, mode, last_modified_ms).await
    }
    ...
    pub async fn pull<W: AsyncWrite + Unpin + Send + 'static>(
        ...
        let pulled_bytes = Self::pull_bytes(stream, remote_path).await?;
        match pulled_bytes {
            Ok(bytes) => {
    ...
    async fn read_response<W: AsyncWrite + Unpin + Send + 'static>(
        ...
        loop {
            let chunk = stream.read_chunk().await?;
            if chunk.is_empty() && buf.is_empty() {
                return Err(AdbError::ConnectionClosed("EOF reading SYNC response".into()));
            }
            buf.extend_from_slice(&chunk);
